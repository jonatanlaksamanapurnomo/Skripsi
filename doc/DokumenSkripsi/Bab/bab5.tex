...
\begin{enumerate}
	...
	Fitur ini bertujuan untuk memvisualisasikan data dalam bentuk \textit{heat map}. Jika pengguna memilih \textit{option} \textit{heat map} dan menekan tombol \textit{visualize}, maka program akan menampilkan data dalam bentuk \textit{heat map} pada objek \textit{map} \ref{fig:heatmapState}.
...
Pengujian functional pada perangkat lunak ini dilakukan dengan menggunakan automated test dengan bantuan \textit{third party app} \textit{cypress.io}. Pengujian dibagi dalam tiga tahap:
...
COMPLETION_STATUS.forEach(status => {
    ...
    \end{lstlisting}
    Pengujian ini akan memiliki hasil seperti \ref{fig:visualRegressionState},
    \begin{figure}[H]
	\centering  
	...
	\newpage\item Pengujian \textit{fucntional} dengan mengunakan metode \textit{http response status} \\
    Pengujian ini bertujuan untuk menguji fungsi \textit{heat map} dan \textit{marker clustering} metode yang digunakan adalah melihat \textit{response} status yang dikeluarkan oleh \textit{node server}. syntax test case yang dibuat akan berbentuk:\\
...
Total pengujian yang berjumlah empat puluh tujuh (47) test case yang sudah dilengkapi dengan bukti \textit{screenshoot} dan \textit{video} yang dapat diakses pada tautan \href{https://dashboard.cypress.io/projects/cgtb3m/runs/1/overview}{dashboard}.
...
\subsubsection{Pengujian 1 Bagaimana pengguaan perangkat lunak KIRI pada saat jam operasional angkutan umum }
...
\subsubsection{Pengujian 2 Bagaimana pengguaan perangkat lunak KIRI pada saat jam non operasional angkutan umum}
